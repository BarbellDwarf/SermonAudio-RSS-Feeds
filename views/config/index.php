    ...
    <div class="panel-body">
        ...
        <?php if (!$hasApiKey): ?>
            <div class="alert alert-info">
                <strong>Tip:</strong> Advanced features like event type filtering and sermon limits are available when you <a href="<?= Url::to(['/admin/module', 'id' => 'sermonaudio']) ?>">configure an API key</a> in the global module settings.
            </div>
        <?php endif; ?>
                ...
                <tbody>
                    <?php foreach ($feeds as $feed): ?>
                        <tr>
                            ...
                            <?php if ($hasApiKey): ?>
                                <td>
                                    <?php if (!empty($feed->event_type)): ?>
                                        <?php 
                                        $standardTypes = array_keys(\app\modules\sermonaudio\models\Feed::getEventTypeOptions());
                                        $isCustom = !in_array($feed->event_type, $standardTypes);
                                        ?>
                                        <span class="label <?= $isCustom ? 'label-warning' : 'label-info' ?>" 
                                              title="<?= $isCustom ? 'Custom event type' : '' ?>">
                                            <?= Html::encode($feed->event_type) ?>
                                        </span>
                                    <?php else: ?>
                                ...
                                <td>
                                    <span class="label label-default"><?= $feed->sermon_limit ?></span>
                                </td>
                            <?php endif; ?>
                            <td>
                                <?php 
...
<div class="panel panel-default">
    ...
    <div class="panel-body">
        <div class="alert alert-info">
            <strong>Note:</strong> By default, feeds are checked once per hour via HumHub's built-in cron.
        </div>
    </div>
</div>
